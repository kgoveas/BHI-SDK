 ...
 *
 * @file             VirtWakeupTemerature.c
 *
 * @brief            Virtual driver for wakeup temperature sensor.
 *
 * @date             May/08/2017
 *
 */
/*!
 * @addtogroup VirtWakeupTemperature
 * @brief
 * @{*/
 ...
 */
static SensorStatus virt_wakeup_temperature_handle_sensor_data(VirtualSensorDescriptor *self, void *data)
{
    PhysicalSensorDescriptor *phys_bme_p = self->triggerSource.phys;
    int32_t *wkup_temperature_phys_p = data; /* physical data */
 ...
 * @return Result of execution
 */
SensorStatus virt_wakeup_temperature_get_last_sensor_data(VirtualSensorDescriptor *self)
{
    PhysicalSensorDescriptor* phys_bme_p = self->triggerSource.phys;
    if (NULL == phys_bme_p)
    {
 ...
 * @return None
 */
static void virt_wakeup_temperature_mode_changed(VirtualSensorDescriptor *self, SensorPowerMode mode)
{
    PhysicalSensorDescriptor *phys_bme_p = self->triggerSource.phys;
    if (NULL == phys_bme_p)
    {
...
VIRTUAL_SENSOR_DESCRIPTOR VirtualSensorDescriptor virt_wakeup_temperature_descriptor =
{
    .triggerSource =
    {
        .sensor =
    ...
    .type =
    {
        .value = SENSOR_TYPE_WAKE_TEMPERATURE,
    ...
    .handle_sensor_data = virt_wakeup_temperature_handle_sensor_data,
    .get_last_sensor_data = virt_wakeup_temperature_get_last_sensor_data,
    .mode_changed = virt_wakeup_temperature_mode_changed,
};//lint !e785
