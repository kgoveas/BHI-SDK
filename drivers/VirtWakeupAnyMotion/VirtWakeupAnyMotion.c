...
/*!
 *@addtogroup VirtWakeupAnyMotion
 *@brief
 *@{*/
 ...
 */
SensorStatus virtual_any_motion_handle_sensor_data(VirtualSensorDescriptor *self, void *data)
{
    UNUSED(data);
    PhysicalSensorDescriptor *phys_any_motion_p = self->triggerSource.phys;
    /* check physical sensor exist or not */
    if (NULL == phys_any_motion_p)
    {
 ...
 * @return SensorOK for sucessful data report. Other values for failure.
 */
static SensorStatus virtual_any_motion_get_last_sensor_data(VirtualSensorDescriptor *self)
{
    PhysicalSensorDescriptor *phys_any_motion_p = self->triggerSource.phys;
    /* check physical sensor exist or not */
    if (NULL == phys_any_motion_p)
...
VIRTUAL_SENSOR_DESCRIPTOR VirtualSensorDescriptor virt_any_motion_descriptor =
{
    .triggerSource =
    {
        .sensor =
        {
            .type =
            {
                .value = SENSOR_TYPE_INPUT_ANY_MOTION,
    ...
    .type =
    {
        .value = SENSOR_TYPE_WAKE_ANY_MOTION,
    ...
    .initialize = NULL,
    .handle_sensor_data = virtual_any_motion_handle_sensor_data,
    .get_last_sensor_data = virtual_any_motion_get_last_sensor_data,
    /* any motion is special sensor type, and the sensor data is event, though need to implement get_lost_sensor_date,
