 ...
 */
static SensorStatus virt_wakeup_exled_init(VirtualSensorDescriptor *self)
{
    UNUSED(self);
    output.led_flag = 0;
    return SensorOK;
...
/*!
 * @brief This function reports the wakeup on/off signal of external led
 *
 * @param[in] self : Descriptor for virtual sensor
 ...
 */
static SensorStatus virt_wakeup_exled_handle_sensor_data(VirtualSensorDescriptor *self, void *data)
{
    PhysicalSensorDescriptor *phys_exled_p = self->triggerSource.phys;
    uint8_t *wkup_led_phys_p = data; /* physical data */
    ...
    if (output.led_flag != *wkup_led_phys_p)
    {
        output.led_flag = *wkup_led_phys_p;
        /* As each event and timestamp are reported, so no need to record the timestamp of the last reported event */
        reportSensorEvent(self, &output.led_flag, self->timestamp);
    }
...
VIRTUAL_SENSOR_DESCRIPTOR VirtualSensorDescriptor virtwakeupexled_descriptor = {
    ...
    .initialize = virt_wakeup_exled_init,
    .handle_sensor_data = virt_wakeup_exled_handle_sensor_data,
    .mode_changed = NULL,
}; //lint !e785
