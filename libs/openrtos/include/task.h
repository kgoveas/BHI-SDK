/*
    OpenRTOS V9.0.0p  Copyright (C) 2016 Real Time Engineers Ltd.
...
static const TaskParameters_t xCheckTaskParameters =
{
    ...
    NULL,       // pxTaskBuffer - the buffer to be used as the task TCB (ignored here)
 ...
 *
 * xTaskCreateRestrictedStatic() should only be used in systems that include an MPU
 * implementation.
 *
 * Create a new task and add it to the list of tasks that are ready to run.
 * The function parameters define the memory regions and associated access
 ...
 * @param pxTaskDefinition Pointer to a structure that contains a member
 * for each of the normal xTaskCreate() parameters (see the xTaskCreate() API
 * documentation) plus a stack buffer and TCB buffer, and the memory region
 * definitions.
 *
 * @param pxCreatedTask Used to pass back a handle by which the created task
 * can be referenced.
...
static const TaskParameters_t xCheckTaskParameters =
{
    ...
    ( 1UL | portPRIVILEGE_BIT ),// uxPriority - task priority, set the portPRIVILEGE_BIT if the task should run in a privileged state.
    cStackBuffer,// puxStackBuffer - the buffer to be used as the task stack.
    xTaskBuffer,// pxTaskBuffer - the buffer to be used as the task TCB (will be accessed in privileged mode only)
