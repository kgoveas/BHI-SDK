...
#define SIMPLE_ERROR_RETURN(code) \
    ...
    while ((instr_ptr < final_instr_ptr) && (!search_over)) {
        ...
        case DW_CFA_info:
            {
                Dwarf_Unsigned arg1;
                DECODE_LEB128_UWORD(instr_ptr, arg1);

                // Arbitrary communcation to me.
                int cannot_know_previous_sp = arg1 & 1;
                int cannot_know_previous_return_addr = arg1 & 2;
                if (cannot_know_previous_sp || cannot_know_previous_return_addr)
                {
                    //printf("\ncannot_know_previous_sp = %d\n", cannot_know_previous_sp);
                    //printf("cannot_know_previous_return_addr = %d\n", cannot_know_previous_return_addr);
                    //return failure;
                    if(cannot_know_previous_return_addr)
                    {
                        // prev return address is ilink, rtie
                        //SIMPLE_ERROR_RETURN(DW_DLE_CFE_INFO_UNKNOWN);
                reg_no = 28;
                ...
                localregtab[reg_no].ru_value_type = DW_EXPR_OFFSET;
                localregtab[reg_no].ru_register =
                    dbg->de_frame_same_value_number + 10;
                localregtab[reg_no].ru_offset_or_block_len = 0;
                fp_register = reg_no;
                    }
                    else
