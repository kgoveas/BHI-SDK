...
# First call, pipes not existing, prompt password
    ...
    if [ -z "$passw" ]; then
        read -s -p $'\nKernel: Enter REST password for user '$user':'$'\n' password
    else
        password=$passw
    fi
